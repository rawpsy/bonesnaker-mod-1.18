...
public class CustomItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        if(Screen.hasShiftDown()) {
            tooltip.add(new TranslatableText("item.snakerbone.custom_item.tooltip.shift"));
        } else {
            tooltip.add(new TranslatableText("item.snakerbone.custom_item.tooltip"));
        }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if(context.getWorld().isClient) {
            ...
            if(!foundBlock) {
                player.sendMessage(new TranslatableText("item.snakerbone.custom_item.no_valuables"), false);
            }
